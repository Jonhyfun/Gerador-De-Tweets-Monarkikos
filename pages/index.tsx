...
export default function Index({monarkisses}) {
  ...
  const [generatedText, setText] = useState(GenerateText(monarkisses,chaos));
  return (
    <div className="w-screen h-screen flex flex-col gap-6 -mt-6 md:-mt-0 md:gap-10 justify-center items-center bg-blue-400">
      ...
      <Tweet setChaos={setChaos} monarkisses={monarkisses} fakeText={generatedText}/>
      <button onClick={() => setText(GenerateText(monarkisses,chaos))} className="hover text-white font-bold py-2 px-4 rounded-full" style={{boxShadow: '2px 4px 10px #00000050', backgroundColor: 'rgb(29, 155, 240)'}}>
        Gerar mais um Tweet!
      </button>
